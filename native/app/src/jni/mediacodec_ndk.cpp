...
static const char *TAG = "MediaCodecNdk";
...
struct syms
{
    struct {
        ...
        pf_AMediaCodec_delete deleteCodec;
    ...
    struct {
        pf_AMediaFormat_new newFormat;
        pf_AMediaFormat_delete deleteFormat;
...
#define OFF(x) offsetof(struct syms, AMediaCodec.x)
    ...
    { "AMediaCodec_delete", OFF(deleteCodec), true },
...
#define OFF(x) offsetof(struct syms, AMediaFormat.x)
    { "AMediaFormat_new", OFF(newFormat), true },
    { "AMediaFormat_delete", OFF(deleteFormat), true },
...
InitSymbols(mc_api *api)
{
    ...
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "function name:%s.\n", members[i].name);
    ...
    ret = i_init_state == 1;
    if (!ret)
        Jni_Print(ANDROID_LOG_INFO, TAG, "MediaCodec NDK init failed.\n");
...
static int Stop(mc_api *api)
{
    ...
    if (p_sys->p_codec)
    {
        ...
        syms.AMediaCodec.deleteCodec(p_sys->p_codec);
        p_sys->p_codec = NULL;
    }
    if (p_sys->p_format)
    {
        syms.AMediaFormat.deleteFormat(p_sys->p_format);
        p_sys->p_format = NULL;
    }

    Jni_Print(ANDROID_LOG_INFO, TAG, "MediaCodec via NDK closed");
    return 0;
}
...
static int Start(mc_api *api, union mc_api_args *p_args)
{
    ...
    if (!p_sys->p_codec)
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.createCodecByName for %s failed",
                api->psz_name);
        goto error;
    }

    p_sys->p_format = syms.AMediaFormat.newFormat();
    if (!p_sys->p_format)
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaFormat.new failed");
        goto error;
    }
    ...
    if (api->i_cat == VIDEO_ES)
    {
        ...
        {
            p_anw = (ANativeWindow *)p_args->video.p_surface;
    ...
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.configure failed");
        goto error;
    }
    if (syms.AMediaCodec.start(p_sys->p_codec) != AMEDIA_OK)
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.start failed");
        goto error;
    }
    ...
    Jni_Print(ANDROID_LOG_INFO, TAG, "MediaCodec via NDK opened");
error:
    if (i_ret != 0)
        Stop(api);
    return i_ret;
 ...
 * DequeueInput
 *****************************************************************************/
static int DequeueInput(mc_api *api, int64_t i_timeout)
{
    mc_api_sys *p_sys = api->p_sys;
    ssize_t i_index;
    ...
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.dequeueInputBuffer failed\n");
        return MC_API_ERROR;
    }
}
...
static int QueueInput(mc_api *api, int i_index, const void *p_buf,
                      size_t i_size, uint64_t i_ts, bool b_config)
{
    mc_api_sys *p_sys = api->p_sys;
    uint8_t *p_mc_buf;
    size_t i_mc_size;
    ...
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.queueInputBuffer failed.\n");
        return MC_API_ERROR;
    }
}
 ...
 * DequeueOutput
 *****************************************************************************/
static int DequeueOutput(mc_api *api, int64_t i_timeout)
{
    mc_api_sys *p_sys = api->p_sys;
    ssize_t i_index;
    ...
    {
        Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.dequeueOutputBuffer failed.\n");
        return MC_API_ERROR;
    }
}
...
static int GetOutput(mc_api *api, int i_index, mc_api_out *p_out)
{
    ...
    if (i_index >= 0)
    {
        ...
        else
        {
            ...
            {
                Jni_Print(ANDROID_LOG_INFO, TAG, "AMediaCodec.getOutputBuffer failed.\n");
                return MC_API_ERROR;
            }
...
static int SetOutputSurface(mc_api *api, void *p_surface, void *p_jsurface)
{
    ...
    return syms.AMediaCodec.setOutputSurface != NULL
        && syms.AMediaCodec.setOutputSurface(p_sys->p_codec, (ANativeWindow *)p_surface)
 ...
 * Configure
 *****************************************************************************/
static int Configure(mc_api * api, const char *psz_name)
{
#if 0
    bool b_adaptive;
    ...
    if (psz_name && strlen(psz_name) > 0) {
        strncpy(api->psz_name, psz_name, sizeof(api->psz_name) -1);
    }
...
int MediaCodecNdk_Init(mc_api *api)
{
    ...
    api->p_sys = (mc_api_sys *)calloc(1, sizeof(mc_api_sys));
    if (!api->p_sys)
        return MC_API_ERROR;
